...
class JarvisAssistant:
    def __init__(self):
        ...
        **Important:** When you need to use one of the available tools to fulfill the user's request, respond ONLY with a single JSON object containing the 'tool_call' details as shown below. Do NOT add any other text before or after the JSON object.
    ...
    def process_command(self, command):
        ...
        # Handle tool execution requests from LLM
            ...
            try:
                ...
                return "Invalid tool call JSON format"
            except Exception as e:
    ...
    def get_response(self, user_message, model="groq"):
        ...
        while iterations < max_tool_iterations:
            iterations += 1
            tool_call_data = None
            # Use regex to find a potential JSON tool call within the response
            # This pattern looks for {"tool_call": { ... }} structure, allowing whitespace and properly handling nested JSON
            # Use the 'final_response' from the previous iteration or the initial response
            # First try a precise pattern that handles nested JSON objects
            tool_call_match = re.search(r'{\s*"tool_call"\s*:\s*{(?:[^{}]|(?:\{[^{}]*\}))*}}', final_response, re.DOTALL)
            
            # If the first pattern doesn't match, try a more comprehensive pattern
            if not tool_call_match:
                # This pattern can handle more complex nested structures with multiple levels
                tool_call_match = re.search(r'{\s*"tool_call"\s*:\s*{.*?}}', final_response, re.DOTALL)
                
            # If still no match, try a very lenient pattern as last resort
            if not tool_call_match:
                # Look for any JSON-like structure that might contain a tool_call
                tool_call_match = re.search(r'{[^{]*"tool_call".*?}', final_response, re.DOTALL)

            if tool_call_match:
                json_text = tool_call_match.group(0)
                try:
                    # Attempt to parse the extracted JSON
                    tool_call_data = json.loads(json_text)
                except json.JSONDecodeError:
                    # If parsing fails, it's not a valid tool call JSON
                    pass # Continue to treat response as non-tool call

            # Check if we successfully parsed a tool call structure
            if isinstance(tool_call_data, dict) and "tool_call" in tool_call_data:
                ...
                if tool_name in self.tools:
                    ...
                    try:
                        ...
                        logger.tool(tool_name, f"Executing with args: {tool_args}")
                    ...
                    # Continue loop to check if the new response is another tool call
                    continue
                else:
                    ...
                    break # Exit loop, treat as final response
            else:
                # Not a valid tool call structure or regex didn't match
                logger.debug("Response is not a tool call, using as-is")
                break # Exit loop, treat as final response
