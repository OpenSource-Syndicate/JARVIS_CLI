...
class HotwordDetector:
        ...
        try:
            ...
            while not self._stop_requested and (stop_event is None or not stop_event.is_set()):
                ...
                if keyword_index >= 0:
                    ...
                    # Reset detector state for next detection
                    self._stop_requested = False
                    return True  # Return True if a hotword is detected

        finally:
            self._cleanup()
            # Reinitialize resources for next detection
            self.porcupine = pvporcupine.create(keywords=self.keywords)
            self.audio_interface = pyaudio.PyAudio()
            self.audio_stream = self.audio_interface.open(
                rate=self.porcupine.sample_rate,
    ...
    def _play_sound(self, sound_path: Optional[str]) -> None:
        """Plays a sound file if a path is provided using pygame."""
        if sound_path:
            try:
                pygame.mixer.init()
                ...
                if self.prints: print(f"Error playing sound: {e}")
